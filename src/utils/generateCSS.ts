...
const stateNames = ["active", "hover", "focus", "disabled"];
...
const isValidCSSValue = (property: string, value: string): boolean => {
  ...
  switch (property) {
    ...
    case "bottom":
    case "left":
      return /^(auto|0|\d+(\.\d+)?(px|rem|em|%|vh|vw|vmin|vmax)|calc\([^)]+\))$/i.test(
        normalizedValue
      );
    ...
    case "margin":
    case "padding":
      return /^(auto|0|\d+(\.\d+)?(px|rem|em|%|vh|vw)|calc\([^)]+\))(\s+(auto|0|\d+(\.\d+)?(px|rem|em|%|vh|vw)|calc\([^)]+\)))*$/i.test(
        normalizedValue
      );
    ...
    case "border-color":
    case "outline-color":
      return /^(transparent|currentColor|[a-z]{3,20}|#[0-9a-f]{3,8}|rgb\(\s*\d+%?\s*,\s*\d+%?\s*,\s*\d+%?\s*\)|rgba\(\s*\d+%?\s*,\s*\d+%?\s*,\s*\d+%?\s*,\s*[01]?\d?\d?\s*\)|hsl\(\s*\d+\s*,\s*\d+%\s*,\s*\d+%\s*\)|hsla\(\s*\d+\s*,\s*\d+%\s*,\s*\d+%\s*,\s*[01]?\d?\d?\s*\))$/i.test(
        normalizedValue
      );

    // 字体相关
    case "font-size":
      return /^(xx-small|x-small|small|medium|large|x-large|xx-large|smaller|larger|\d+(\.\d+)?(px|rem|em|%))$/i.test(
        normalizedValue
      );
    case "font-weight":
      return /^(normal|bold|bolder|lighter|100|200|300|400|500|600|700|800|900)$/i.test(
        normalizedValue
      );
    case "font-family":
    ...
    case "text-decoration":
      return /^(none|underline|overline|line-through|blink)$/i.test(
        normalizedValue
      );

    // 布局相关
    case "display":
      return /^(block|inline|inline-block|flex|inline-flex|grid|inline-grid|none|table|inline-table)$/i.test(
        normalizedValue
      );
    case "position":
    ...
    case "justify-content":
      return /^(flex-start|flex-end|center|space-between|space-around|space-evenly)$/i.test(
        normalizedValue
      );
    case "align-items":
      return /^(stretch|flex-start|flex-end|center|baseline)$/i.test(
        normalizedValue
      );
    ...
    case "border":
    case "border-width":
      return /^(thin|medium|thick|\d+(\.\d+)?(px|rem|em))$/i.test(
        normalizedValue
      );
    case "border-style":
      return /^(none|hidden|dotted|dashed|solid|double|groove|ridge|inset|outset)$/i.test(
        normalizedValue
      );
    ...
    // 动画相关
    case "animation":
      return /^[a-zA-Z-]+\s+(\d+(\.\d+)?(s|ms))?\s*([a-zA-Z-]+)?\s*(\d+)?\s*([a-zA-Z-]+)?$/i.test(
        normalizedValue
      );

    // 变换相关
    case "transform":
      return /^(none|translate(X|Y|Z|3d)?\([^)]+\)|rotate(X|Y|Z)?\([^)]+\)|scale(X|Y|Z)?\([^)]+\)|skew(X|Y)?\([^)]+\)|matrix(3d)?\([^)]+\))(\s+(translate(X|Y|Z|3d)?\([^)]+\)|rotate(X|Y|Z)?\([^)]+\)|scale(X|Y|Z)?\([^)]+\)|skew(X|Y)?\([^)]+\)|matrix(3d)?\([^)]+\)))*$/i.test(
        normalizedValue
      );
...
export function generateCSS(classNames: string[]): string {
  ...
  classNames.forEach((cName) => {
    ...
    // 处理状态类Hover, focus, and other states
    ...
    // 过滤无效的状态类
    if (stateName && !stateNames.includes(stateName)) return;
    ...
    if (propertyPrefix === "animation") {
      ...
      if (PREDEFINED_KEYFRAMES[name as keyof typeof PREDEFINED_KEYFRAMES]) {
        ...
        cssObjValue = `{ animation: ${cssValue} }`;
      }
      // 验证CSS属性
    } else if (
      ...
      cssObjValue = `{ ${propertyPrefix}: ${cssValue} }`;
    }
    ...
    if (cssObjValue) {
      ...
      if (stateName) {
        cssParts.push(
          `.${stateName}\\:${escapedName}:${stateName} ${cssObjValue}`
        );
        // 处理非状态类
      } else {
        cssParts.push(`.${escapedName} ${cssObjValue}`);
      }
    }
  });
